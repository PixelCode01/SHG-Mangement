...
// Simple period interest calculation function for testing
...
async function testPeriodBasedInterestCalculation() {
  try {
    ...
    testCases.forEach((testCase, index) => {
      ...
      const newCalculation = calculatePeriodInterest(testCase.loanAmount, testCase.annualRate, testCase.frequency);
    ...
    if (testGroup) {
      ...
      testGroup.memberships.forEach(membership => {
        ...
        if (currentLoanBalance > 0) {
          ...
          const newInterest = calculatePeriodInterest(currentLoanBalance, testGroup.interestRate || 0, testGroup.collectionFrequency);
      ...
      console.log('No group with interest rate found in database');
    }
