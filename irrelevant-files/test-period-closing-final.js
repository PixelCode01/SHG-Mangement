...
async function testPeriodClosingDirect() {
  ...
  try {
    ...
    const paidContributions = openPeriod.memberContributions.filter(c => c.status === 'PAID' || c.remainingAmount === 0);
    const totalCollection = openPeriod.memberContributions
      .filter(c => c.status === 'PAID' || c.remainingAmount === 0)
    ...
    console.log(`💰 Financial Standing Calculation:
      Previous: Bank ₹${bankBalance}, Cash ₹${cashInHand}
      Current Collection: ₹${totalCollection}
      New: Bank ₹${newBankBalance}, Cash ₹${newCashInHand}
      Total Loans Outstanding: ₹${totalLoanAssets}`);
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.groupPeriodicRecord.update({
        ...
        data: {
          ...
          totalGroupStandingAtEndOfPeriod: newBankBalance + newCashInHand + totalLoanAssets
        }
      });
      ...
      const nextPeriod = await tx.groupPeriodicRecord.create({
        data: {
          ...
          standingAtStartOfPeriod: newBankBalance + newCashInHand + totalLoanAssets
        }
      });
    ...
    for (let i = 0; i < memberContributions.length; i += batchSize) {
      ...
      await prisma.$transaction(async (tx) => {
        for (const contribution of batch) {
          if (contribution.status === 'PAID' || contribution.remainingAmount === 0) {
